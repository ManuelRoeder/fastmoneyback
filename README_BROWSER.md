# Fast Money Back — Browser-Version (ohne Java)

Der Antrag auf Abschlagszahlung wird jetzt **vollständig im Browser** erstellt.
Kein Java, keine Installation, kein Server — und die Finanz-PDFs verlassen den
Rechner nicht.

## Was sich geändert hat
- **Java entfernt.** Die Tabellen werden nicht mehr mit `tabula-py` (Java) gelesen,
  sondern mit **pdfplumber** (reines Python). Getestet mit blockiertem `tabula`-Import.
- **Alles im Speicher.** Sämtliche Funktionen arbeiten auf Bytes statt auf Dateipfaden,
  damit derselbe Code am Desktop *und* im Browser läuft.
